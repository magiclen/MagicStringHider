...
public class StringHider {
	...
	if (originalHiddenString != null && originalHiddenString.length() > 0) {
		...
		try (ByteArrayOutputStream bais = new ByteArrayOutputStream()) {
			...
			gos.write(originalHiddenString.getBytes());
		    }
	...
	if (compressedHiddenString != null && compressedHiddenString.length() > 0) {
	    try {
		final byte[] textByte = compressedHiddenString.getBytes();
		if (textByte[0] != 'X') {
		    sb.append(compressedHiddenString);
		} else {
		    final byte[] bytes = new byte[(textByte.length - 1) / 2];
		    int p = 0;
		    for (int i = 1; i < textByte.length; i += 2) {
	...
	return hideString(originalString, null);
    }
	...
	if (originalString != null && originalString.length() > 0) {
		...
		for (int i = 0; i < textByte.length; i += 2) {
		...
		if (key != null) {
			...
			final int rand = (strLength * i) % 3;
			switch (rand) {
			    case 0:
				sb.append((char) ((int) (Math.random() * 23) + 'A'));
				break;
			    case 1:
				final int rndLength = (int) (Math.random() * 3 + 1);
				sb.append('Z').append(hideInt(rndLength, rnd));
				for (int j = 0; j < rndLength; j++) {
				    sb.append((char) ((int) (Math.random() * 23) + 'A'));
				}
				break;
			    case 2:
				final int rndLength1 = (int) (Math.random() * 3 + 1);
				final int rndLength2 = (int) (Math.random() * 3 + 1);
				sb.append('Y').append(hideInt(rndLength1, rnd)).append(hideInt(rndLength2, rnd));
				for (int j = 0; j < rndLength1; j++) {
				    sb.append((char) ((int) (Math.random() * 23) + 'A'));
				}
				for (int j = 0; j < rndLength2; j++) {
				    sb.append((char) ((int) (Math.random() * 23) + 'A'));
				}
				break;
			}
	...
	if (hiddenString != null && hiddenString.length() > 0) {
		...
		for (int i = 0; i < textByte.length; i += 2) {
		    ...
		    } else if (i < (intLength) * 2) { // 計算原字串的長度
			length = length * 10 + textByte[i + 1] - 'A';
		    } else if (i == (intLength) * 2) { // 計算原字串的長度結束
			length = length * 10 + textByte[i + 1] - 'A';
			...
			rnd = (textByte[rndPos + 1] - 'A') * 10
				+ textByte[rndPos + 3] - 'A';
			for (int j = rndPos; j < textByte.length - 4; j++) {
			    textByte[j] = textByte[j + 4];
			}
			rndPos = textByte.length - 4;
		    } else if (i != rndPos && i != rndPos + 2) {
			if (textByte[i + 1] == 'Z') { // 英文或其他半形文字
			    i += 2;
			...
			} else if (textByte[i + 1] == 'Y') { // 中文或其他全形文字
			    i += 2;
	...
	if (hiddenString != null && hiddenString.length() > 0 && sb.length() == 0) {
	    return null;
	} else {
	    return sb.toString();
	}
