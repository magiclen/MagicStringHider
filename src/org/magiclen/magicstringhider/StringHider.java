...
public class StringHider {
	...
	if (originalString != null && originalString.length() > 0) {
		...
		if (addValidation) {
		    ...
		    while (stringValue > 0) {
			final char temp = hideHide(hideInt(stringValue % 10, rnd), hashValue);
			sb.append(temp);
			stringValue /= 10;
		    }
		}
    ...
    private static char hideHide(final char str, final int hashValue) {
	if (hashValue != 23) {
